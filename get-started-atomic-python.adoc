...
Now the container will no longer be running which can be verified with `docker ps -a`. Containers that are no longer needed can be cleaned up with `docker rm <container-name>`.
