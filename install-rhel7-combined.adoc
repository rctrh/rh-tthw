...
. When `virt-manager` opens, it will show the available hypervisor connections. If you don't see _QEMU/KVM_ list, select _New Connection_ from the _File_ menu. Select _QEMU/KVM_ from the list of hypervisors. Then click _Connect_.
...
.. Select _Local install media_ (ISO image or CDROM)_ under _Choose how you would like to install the operating system_.
.. Click _Forward_.
+
...
.. Click _Browse..._
... Click _Browse Local_ on the _Choose Storage Volume_ dialog.
... Navigate to the Red Hat Enterprise Linux Server DVD `.iso` file you downloaded in step 1 of this guide.
.. Uncheck _Automatically detect operating system based on install media.
