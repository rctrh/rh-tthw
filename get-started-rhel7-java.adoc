...
In this tutorial, you will see how to get started with Java development on Red Hat Enterprise Linux by creating a simple Hello World application. You will install a Java Development Kit (JDK), and learn which Java packages are available.  The whole tutorial should take less than 10 minutes to complete.
...
The output will show the package name and on which software repository it resides in. Only software repositories that have been enabled for downloading are included in the results.
...
To see which repositories are currently have enabled:
...
. How do I find out which JRE and JDKs are already installed?
+
