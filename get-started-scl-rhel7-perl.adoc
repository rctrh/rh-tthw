...
Under your normal user id, start a terminal session.  First, we will use scl enable to add Perl 5.20 to your environment. Then we will run perl -v to check the version number.
...
Now we will create a Perl program that can be run from the command line. Using your preferred text editor, create hello.pl: 
...
If you get an error that Perl is not found, or see an older version number, you need to run `scl enable rh-perl520 bash` first.
