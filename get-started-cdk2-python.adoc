...
. Unzip cdk-2.0.0.zip into a directory of your choice. 
...
. Add the RHEL 7.2 Vagrant box (rhel-cdk-kubernetes) to Vagrant with the command `vagrant add --name cdkv2 _path to saved .box file_`.
...
This step will download and install Python 3 using a container from the Red Hat Atomic Registry, a repository of container images. Installing the Python 3 container will make Python 3 available for use by other containers on your system. Because containers run in isolated environments, your host system will not be altered by the installation. You must use `docker` commands to use or view the container's content.
